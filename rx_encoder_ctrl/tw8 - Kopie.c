...
int	tw8_config(int chip, int speed_mminn)
{
	...
	// cfg.MAIN_INTER
	cfg.MAIN_INTER			= 0x7D0; // 16bit // inter = MAIN_INTER/4 // AB Output Resolution in Edges
	...
	cfg.MAIN_FLTR.p			= 0x2; // 0x0; // 3bit // Output Filder Proportional Gain
	...
	cfg.AB_CFG0.apol		= 0x1; // 1bit // A Output Polarity / State at Z Position
	cfg.AB_CFG0.bpol		= 0x0; // 1bit // B Output Polarity / State at Z Position
	...
	// cfg.AB_ZWIDTH
	cfg.AB_ZWIDTH			= 0x00;  // 8bit // 0x00=Z-Output is off // zwidth = (AB_ZWIDTH + 1) / (AB_CFG1.div + 1) //  Z Output Pulse Width
	// cfg.VTOP
	cfg.AB_VTOP				= 0x3; // 7bit // fab=fcore/(4 x (AB_VTOP + 1) x (AB_CFG1.div + 1)) // 0x03; // max 0x7F //  AB Output Frequency Limit
	...
	cfg.ADPT_CFG.aoff		= 0x0; // 0x1; // 1bit // Analog Offset
	cfg.ADPT_CFG.again		= 0x0; // 0x1; // 1bit // Analog Gain
	...
	cfg.ADPT_STORE.offth	= 0x00; // 4bit // Dig. Offs. Threshold for Auto-Storage to EEPROM during operation
	cfg.ADPT_STORE.gainth	= 0x00; // 4bit //  Dig. Gain Threshold for Auto-Storage to EEPROM during operation
	...
	cfg.ADPT_CORR.offtol	= 0x02; // 3bit // Auto-Adaption Dig. Offset Tolerance // maximum uncorrected(residual) value that is tolerated before correction 
	cfg.ADPT_CORR.gaintol	= 0x02; // 3bit // Auto-Adaption Dig. Gain Tolerance // maximum uncorrected(residual) value that is tolerated before correction 
	...
	cfg.FLT_CFG.hold		= 0x00;  // 2bit // AB Output Hold during Fault
...
static void _tw8_display_status(void)
{
	...
	/*
	test++;
	tw8_write(0, 0x8000, sizeof(UINT16), &test);
	tw8_write(1, 0x8000, sizeof(UINT16), &test);
	if (test >= 0xFF)
		test = 0;
	*/
}
...
#define ALL for(chip=0; chip<CHIP_CNT; chip++)
	...
	_print_addr("MAIN_CFG",		&_Var[0]->STAT_SP);		ALL term_printf("0x%04X  ", _Var[chip]->STAT_SP);	term_printf("\n");
	_print_addr("STAT_EE",		&_Var[0]->STAT_EE);		ALL term_printf("0x%04X  ", _Var[chip]->STAT_EE);	term_printf("\n");
