	...
	if (_Init && RX_EncoderCfg.printerType==printer_test_table)
	{
		...
		if (Fpga->cfg.encIn[0].enable)
		{
			...
			if (_UV_Stopping && actPos<UV_POS_LEFT) 
			{
				_UV_Stopping = FALSE;
				_UV_Printing = FALSE;
                fpga_encoder_enable(FALSE, FALSE);
            }
...
{
	int i;
    if (RX_EncoderCfg.printerType == printer_test_table && _UV_Printing)
        _UV_Stopping = TRUE;
    else
    {
        if (RX_EncoderCfg.printerType != printer_test_table)
...
//	Error(LOG, 0, "fpga_pg_config RX_EncoderStatus.distTelCnt=%d", RX_EncoderStatus.distTelCnt);
	...
	if (restart) 
		act_pos = Fpga->stat.encIn[0].position;
	else		 
	{
	...
	
	{
		...
		Fpga->cfg.general.shift_delay = 1;
		if (pcfg->printGoOutDist > 0)
		{			
	...
	for (pgNo=0; pgNo<SIZEOF(Fpga->cfg.pg); pgNo++)
	{
		...
		else 
		{
			...
			default:						Fpga->cfg.general.shift_delay_tel = 0; Fpga->cfg.pg[pgNo].fifos_used = FIFOS_DIST; break;
			}
		}
	}
...
{		
	if (_Socket!=INVALID_SOCKET && !_PrintGo_Locked)
	{
		...
		INT32 diff = Fpga->stat.encOut[0].pg_start_pos-Fpga->stat.encIn[0].position;
		int can_start = ((Fpga->stat.encOut[0].pg_start_pos | Fpga->stat.encIn[0].position)==0) || (diff>0);
		...
		if (Fpga->cfg.pg[0].fifos_used==FIFOS_MARKREADER || Fpga->cfg.pg[0].fifos_used==FIFOS_MARKFILTER)
		{
			...
			{
				if (Fpga->stat.encOut[0].position==0)
				{
					_PM_Cnt=Fpga->stat.encOut[0].PG_cnt;
				}
				else
				{
					{ //---  LOG -----------------------------------------
			...
			{				 
				...
				TrPrintfL(TRUE, "Fpga->stat.encOut[0].mark_edge_warn=%d _PM_Filtered_Cnt=%d, _PM_Cnt=%d, distTelCnt=%d, window_mark_pos=%d", Fpga->stat.encOut[0].mark_edge_warn, _PM_Filtered_Cnt, _PM_Cnt, RX_EncoderStatus.distTelCnt, Fpga->stat.encOut[0].window_mark_pos);
...
static void _fpga_display_status(int showCorrection, int showParam)
{
	...
	if(_ALL){term_printf("  printgo_n:     "); for (i=0; i<cnt; i++)term_printf("%09d  ", Fpga->cfg.pg[i].printgo_n);			term_printf("\n");}
		...
		if (!showCorrection && !showParam)
		{
			...
			term_printf("  enable:        "); for (i=0; i<cnt; i++) term_printf("% 9d  ", Fpga->cfg.encIn[i].enable);			term_printf("\n");
			...
			term_printf("  pos_pg_fwd:    "); for (i=0; i<cnt; i++) term_printf("%09d  ", Fpga->cfg.pg[i].enc_start_pos_fwd);	term_printf("\n");
			term_printf("  pos_pg_bwd:    "); for (i=0; i<cnt; i++) term_printf("%09d  ", Fpga->cfg.pg[i].enc_start_pos_bwd);	term_printf("\n");
		//	term_printf("  enc_start_bwd: "); for (i=0; i<cnt; i++) term_printf("%09d  ", Fpga->cfg.pg[i].enc_start_pos_bwd);	term_printf("\n");
	...
	//		term_printf("  rev_sum:       "); for (i = 0; i < cnt; i++) term_printf("%09d  ", Fpga->stat.encIn[i].rev_sum); term_printf("\n");
			...
			term_printf("  incPerRev cfg: "); for (i = 0; i < cnt; i++) term_printf("%09d  ", Fpga->cfg.encIn[i].inc_per_revolution); term_printf("\n");
			term_printf("  incPerRev stat:"); for (i = 0; i < cnt; i++) term_printf("%09d  ", Fpga->stat.encIn[i].inc_per_revolution); term_printf("\n");
...
//			term_printf("  lin diff err:  "); for (i = 0; i < 1; i++) term_printf("%09d  ", (INT16)Fpga->stat.encIn[i].enc_diff_overflow); term_printf("\n");
			...
			term_printf("  corr_rotative: "); for (i=0; i<4; i++)   term_printf("%09d  ", Fpga->stat.rol_coeff_at_use[i]);		term_printf("\n");
...
{	
	...
	for (i=0; i<2; i++)
	{
		if(!reset && Fpga->cfg.encIn[i].enable && Fpga->cfg.encIn[i].index_en)
		{
			...
			if(p < CHECK_POS && (_pos[i] >= CHECK_POS))
			{
				if(Fpga->stat.encIn[i].rev_sum == 0)						ErrorFlag(ERR_CONT,  &_ErrFlags, 1<<0, 0, "Encoder %d: No Index", i);
			}
			
			//---  check if all indexes detected (if not in rollback) ------------------------
			if (_pos[i]-1000>p && !_Restarted)
			{
				if(Fpga->cfg.encIn[0].inc_per_revolution == INC_PER_REV) 
				{
					...
					{
						if(_CorrRotative) ErrorFlag(ERR_CONT, &_ErrFlags, 1<<1, 0, "Encoder %d: Counting %d Indexes (expected %d)", i, Fpga->stat.encIn[i].rev_sum-_RevSumStart[i], n);
						else			  ErrorFlag(WARN,	  &_ErrFlags, 1<<1, 0, "Encoder %d: Counting %d Indexes (expected %d)", i, Fpga->stat.encIn[i].rev_sum-_RevSumStart[i], n);
					}
				}
				else
				{
					if(_CorrRotative) ErrorFlag(ERR_CONT, &_ErrFlags, 1<<2, 0, "Encoder %d: Counting %d increments per revolution (expected %d)", i, Fpga->cfg.encIn[0].inc_per_revolution, INC_PER_REV);
					else			  ErrorFlag(WARN,	  &_ErrFlags, 1<<2, 0, "Encoder %d: Counting %d increments per revolution (expected %d)", i, Fpga->cfg.encIn[0].inc_per_revolution, INC_PER_REV);
				}				
			}
			if (_Restarted)
			{
