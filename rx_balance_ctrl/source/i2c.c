...
{ 
  ...
  do
  {
		...
		else if (pstcI2C->IBSR & 0x40)  // RACK == 1? ->  busy or error
		{
			...
			if (u16ErrorCount > TIME_OUT)	return -1;
		}
		else
		{
			u16ErrorCount = 0; 
 ...
 **  \retval      -5   Trx Error
 ******************************************************************************/
int I2cSendByte(volatile stc_mfsn_t* pstcI2C, uint8_t u8Data)
{
  int  timeout;
  ...
  pstcI2C->IBCR = 0xB0;   // WSEL = 1, ACKE = 1, Clear INT flag
	...
	for(timeout=TIME_OUT_INT; timeout; timeout--)
	{
		if(pstcI2C->IBCR & 0x01) // Wait for transmission complete via INT flag
		{
			return 0;
		}
