...
#define START_PURGE     0x6503
#define START_BACK      0x6603
#define START_TABLE     0x6703
#define STOP            0x0003
...
static int _Move_Waste_TargetPosition = 678;        // mm
static int _Move_Jet_TargetPosition = 589;          // mm
static int _Move_Back_TargetPosition = 750;         // mm
static int _Move_Purge_TargetPosition = 643;        // mm

static char _LastCommand[256];
...
int drive_init(void)
{
    ...
    {
        _DriveStatus.enabled = rx_def_is_tts(RX_Config.printer.type);
        _DriveStatus.acceleration = 3600;
        _DriveStatus.decelerlation = 3600;
        _DriveStatus.jerk = 96000;

#ifndef linux
        if (_DriveStatus.enabled) Error(ERR_CONT, 0, "Parkem Drive is not implemented in Windows version");
#endif // !linux
    }
...
{
#ifdef linux
    if (rx_def_is_tts(RX_Config.printer.type) && _Error)
    {
        ...
        if (_DriveStatus.enabled)
        {
            ...
            if (_Handle <= 0)
            {
                ...
                else if (!err)
                {
                    ...
                    TrPrintfL(TRUE, "Parkem Drive: Error %d: %s\n", errno, buf);
                }
            }
            ...
            if (_Handle >= 0)
            {
                ...
                {
                    ret = _set_setting(PRINTING, TARGETPOSITION, _DriveStatus.targetposition/*_Move_Back_TargetPosition*/);
                    if (ret == REPLY_OK)
                ...
                {
                    ret = _set_setting(PRINTING, ACCEL, _DriveStatus.acceleration);
                    if (ret == REPLY_OK)
                ...
                {
                    ret = _set_setting(PRINTING, DECEL, _DriveStatus.decelerlation);
                    if (ret == REPLY_OK)
                ...
                if (_Init_Start == 0)
                {
                    ...
                    ret += _set_setting(MOVE_START, TARGETPOSITION, _Move_Start_TargetPosition);
                    ret += _set_setting(MOVE_START, MODE, MOVE_ABS);
                    ret += _set_setting(MOVE_START, ACCEL, _Move_Start_Acceleration);
                    ret += _set_setting(MOVE_START, DECEL, _Move_Start_Acceleration);
                    ret += _set_setting(MOVE_START, JERK, _Move_Start_Jerk);
                    if (ret == REPLY_OK)
                        _Init_Start++;
                    else
                        Error(ERR_CONT, 0, "Parkem Init Error in State %d with %d Errors", _Init_Start, ret);
                }

                if (_Init_Start == 1)
                {
                    ret = _set_setting(MOVE_WASTE, SPEED, _Move_Start_Speed);
                    ret += _set_setting(MOVE_WASTE, TARGETPOSITION, _Move_Waste_TargetPosition);
                    ret += _set_setting(MOVE_WASTE, MODE, MOVE_ABS);
                    ret += _set_setting(MOVE_WASTE, ACCEL, _Move_Start_Acceleration);
                    ret += _set_setting(MOVE_WASTE, DECEL, _Move_Start_Acceleration);
                    ret += _set_setting(MOVE_WASTE, JERK, _Move_Start_Jerk);
                ...
                if (_Init_Start == 3)
                {
                    ret = _set_setting(MOVE_PURGE, SPEED, _Move_Start_Speed);
                    ret += _set_setting(MOVE_PURGE, TARGETPOSITION, _Move_Purge_TargetPosition);
                    ret += _set_setting(MOVE_PURGE, MODE, MOVE_ABS);
                    ret += _set_setting(MOVE_PURGE, ACCEL, _Move_Start_Acceleration);
                    ret += _set_setting(MOVE_PURGE, DECEL, _Move_Start_Acceleration);
                    ret += _set_setting(MOVE_PURGE, JERK, _Move_Start_Jerk);
                ...
                if (_Init_Start == 7)
                {
                    ...
                    if ((_MoveCmd == move_start || _MoveCmd == move_print ||
                        _MoveCmd == move_waste || _MoveCmd == move_waste || _MoveCmd == move_purge ||
                    ...
                    case move_print:
                        if (RX_StepperStatus.info.z_in_print)
                        {
                            ...
                            if (abs(ret - _Move_Start_TargetPosition) <= 2 && _Axis_Stillstand)
                            {
                                _set_setting(CTRL, CONTROL_WORD, STOP);
                    ...
                    case move_purge:
                        _set_setting(CTRL, CONTROL_WORD, STOP);
                        _set_setting(CTRL, CONTROL_WORD, START_PURGE);
                        _MoveCmd = move_run;
                        _Ref_MoveCmd = move_undef;
                        break;
                    ...
                    default:
                        Error(ERR_CONT, 0, "Unknown command %d for drive", _MoveCmd);
                        break;
                    }
                }
            }
...
{
    if (RX_StepperStatus.info.z_in_up || RX_StepperStatus.info.z_in_print || RX_StepperStatus.info.z_in_ref)
    {
        if (_MoveCmd == move_standstill || _MoveCmd == move_wait)
            _MoveCmd = move_waste;
    }
...
int drive_in_jet(void)
{
    ...
    if (abs(pos - _Move_Jet_TargetPosition) <= 1) return TRUE;
    return FALSE;
}
...
static void _send_data(char* data, int len)
{
    ...
    buf[len++] = '\r';                        // CR
    buf[len++] = '\n';                        // LF
    memcpy(_LastCommand, buf, sizeof(_LastCommand));
...
static int _read_data(int function, float* data)
{
    ...
    if (function == WRITE_REGISTER)
    {
        ...
        {
            Error(ERR_CONT, 0, "Error message %s from Parkem Drive", &buf[2]);
            Error(LOG, 0, "Last Message %s to Parkem Drive", _LastCommand);
            return REPLY_ERROR;
        }
    }
...
//--- _drive_load_material -------------------------------
static void _drive_load_material(char* material)
{
    if (rx_def_is_tts(RX_Config.printer.type)) plc_load_material(material);
}
