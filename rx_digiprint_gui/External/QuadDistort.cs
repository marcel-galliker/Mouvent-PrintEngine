...
namespace RX_LabelComposer.External
{
    public class QuadDistort
    {
        ...
        public static Bitmap Distort(Bitmap sourceBitmap, Point topleft, Point topright, Point bottomleft, Point bottomright, int interpolation)
        {
            ...
            if (rotate)
            {
                sourceWidth  = sourceBitmap.Height;
                sourceHeight = sourceBitmap.Width;
            }
            else
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int sx = 0; sx < interpolation; sx++)
                    {
                        for (int sy = 0; sy < interpolation; sy++)
                        {
                            ...
                            if (rotate && x0 > 0 && y0 > 0 && x0 < sourceWidth && y0 < sourceHeight)
                            {
                                c = sourceBmp.GetPixel((int)(sourceHeight-y0), x0);
                            ...
                            else if (!rotate && x0 > 0 && y0 > 0 && x0 < sourceWidth && y0 < sourceHeight)
                            {
...
//                    if (source[0, 0, 3] != 0) //pixel has color
                    if (source[0, 0, 3]>0 && source[0, 0, 3]<=255) //pixel has color
