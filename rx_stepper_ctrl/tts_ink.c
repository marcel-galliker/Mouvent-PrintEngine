...
void tts_ink_main(int ticks, int menu)
{
	...
	{
		if (!(Fpga.par->output & _RO_IPA_C1_OUT || Fpga.par->output & _RO_XL_C1_OUT)) RX_StepperStatus.inkinfo.waste_valve_0 = TRUE;
		if (!(Fpga.par->output & _RO_IPA_C2_OUT || Fpga.par->output & _RO_XL_C2_OUT)) RX_StepperStatus.inkinfo.waste_valve_1 = TRUE;
		if (Fpga.par->output & _RO_IPA_C1_IN) RX_StepperStatus.inkinfo.ipa_valve_0 = TRUE;
		if (Fpga.par->output & _RO_IPA_C2_IN) RX_StepperStatus.inkinfo.ipa_valve_1 = TRUE;
		if (Fpga.par->output & _RO_XL_C1_IN) RX_StepperStatus.inkinfo.xl_valve_0 = TRUE;
		if (Fpga.par->output & _RO_XL_C2_IN) RX_StepperStatus.inkinfo.xl_valve_1 = TRUE;
		if (Fpga.par->output & _RO_FLUSH_C1) RX_StepperStatus.inkinfo.flush_valve_0 = TRUE;
		if (Fpga.par->output & _RO_FLUSH_C2) RX_StepperStatus.inkinfo.flush_valve_1 = TRUE;
	}
	else if (RX_StepperCfg.boardNo == 2)
	{
		if (!(Fpga.par->output & _RO_IPA_C1_OUT || Fpga.par->output & _RO_XL_C1_OUT)) RX_StepperStatus.inkinfo.waste_valve_2 = TRUE;
		if (!(Fpga.par->output & _RO_IPA_C2_OUT || Fpga.par->output & _RO_XL_C2_OUT)) RX_StepperStatus.inkinfo.waste_valve_3 = TRUE;
		if (Fpga.par->output & _RO_IPA_C1_IN) RX_StepperStatus.inkinfo.ipa_valve_2 = TRUE;
		if (Fpga.par->output & _RO_IPA_C2_IN) RX_StepperStatus.inkinfo.ipa_valve_3 = TRUE;
		if (Fpga.par->output & _RO_XL_C1_IN) RX_StepperStatus.inkinfo.xl_valve_2 = TRUE;
		if (Fpga.par->output & _RO_XL_C2_IN) RX_StepperStatus.inkinfo.xl_valve_3 = TRUE;
		if (Fpga.par->output & _RO_FLUSH_C1) RX_StepperStatus.inkinfo.flush_valve_2 = TRUE;
		if (Fpga.par->output & _RO_FLUSH_C2) RX_StepperStatus.inkinfo.flush_valve_3 = TRUE;
	}
	...
	case CMD_FLUID_TTS:
		...
		case 2:
			...
			case 3:
				...
				break;
			default:
				Error(ERR_CONT, 0, "Unknown command %d for Fluid System", value.value);
				break;
			}
			break;
		case 1:
