...
int tts_ink_handle_ctrl_msg(RX_SOCKET socket, int msgId, void *pdata)
{
    ...
    case CMD_TTS_C3_ISOP_XL:
        ...
        else if (Fpga.par->output & _RO_IPA_C1_IN)
        {
            Fpga.par->output &= ~RO_XL_C1_OUTPUT;
            Fpga.par->output ^= RO_IPA_C1_OUTPUT;
        }
    ...
    case CMD_TTS_C2_ISOP_XL:
    case CMD_TTS_C4_ISOP_XL:
        if (Fpga.par->output & _RO_XL_C2_IN)
        {
            Fpga.par->output &= ~RO_IPA_C2_OUTPUT;
            Fpga.par->output ^= RO_XL_C2_OUTPUT;
        }
        else if (Fpga.par->output & _RO_IPA_C2_IN)
        {
            Fpga.par->output &= ~RO_XL_C2_OUTPUT;
            Fpga.par->output ^= RO_IPA_C2_OUTPUT;
        }
    ...
    case CMD_TTS_WASTE_VALVE:
        pos = *((INT32 *)pdata);

        if (_Cluster1_Valve_Input != (pos & 0x00000003))
        {
            _Cluster1_Valve_Input = pos & 0x00000003;
            switch (_Cluster1_Valve_Input)
            {
            case 0:
                Fpga.par->output &= ~RO_C1_ALL;
                break;
        ...
        if (_Cluster2_Valve_Input != (pos & 0x0000000c) >> 2)
        {
            _Cluster2_Valve_Input = (pos & 0x0000000c) >> 2;
            switch (_Cluster2_Valve_Input)
            {
            case 0:
                Fpga.par->output &= ~RO_C2_ALL;
                break;
