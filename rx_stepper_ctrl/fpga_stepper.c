...
if (RX_StepperCfg.printerType==printer_LB702_UV || RX_StepperCfg.printerType==printer_LB702_WB || RX_StepperCfg.printerType==printer_LB703_UV || RX_StepperCfg.printerType==printer_LH702 || RX_StepperCfg.printerType==printer_DP803) 
...
static void  _check_errors(void)
{
	if (Fpga.stat->voltage_3v3!= INVALID_VALUE && _ErrorFlags==0)
	{
		...
		if (_ErrorCnt[5]==5) _ErrorFlags=Error(LOG, 0, "Temperature Error: Temp Level is %d °C, row=%d", temp, rowTemp);	
	}	
}
