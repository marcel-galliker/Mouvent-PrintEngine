...
// #endif
			else
			{
				...
				if (motor_move_to_step(MOTOR_Z_FRONT, &_ParZ_down, _z_micron_2_steps(REF_HEIGHT - _PrintPos)) && 
	...
	case CMD_LIFT_CAPPING_POS:		strcpy(_CmdName, "CMD_LIFT_CAPPING_POS");
		...
		{
			RX_StepperStatus.cmdRunning = msgId;
			if (motor_move_to_step(MOTOR_Z_FRONT, &_ParZ_cap, _z_micron_2_steps(CAPPING_HEIGHT)) &&		
