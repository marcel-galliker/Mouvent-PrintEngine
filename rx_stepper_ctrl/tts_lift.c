...
#define POS_JET_TRAY -1800 // um

#define CLEAR_ERROR_TIME 1000 // ms
#define PURGE_PUMP_TIME 30000 // ms
...
void tts_lift_main(int ticks, int menu)
{
	...
	if (RX_StepperStatus.cmdRunning && motors_move_done(MOTOR_Z_BITS))
	{
		...
		RX_StepperStatus.info.z_in_cap = (RX_StepperStatus.cmdRunning == CMD_LIFT_CAPPING_POS && RX_StepperStatus.info.ref_done);
		RX_StepperStatus.info.z_in_jet = (RX_StepperStatus.cmdRunning == CMD_LIFT_JET_TRAY && RX_StepperStatus.info.ref_done);

		if (RX_StepperStatus.cmdRunning == CMD_LIFT_REFERENCE && _PrintPos_New && RX_StepperStatus.info.ref_done)
		{
		...
		else if (_NewCmd)
		{
			...
			case CMD_LIFT_UP_POS:
				...
				tts_lift_handle_ctrl_msg(INVALID_SOCKET, local_new_cmd, NULL);
				break;
			default:
				Error(ERR_CONT, 0, "Command %08x not implemented", local_new_cmd);
				break;
			}
		}
		else
...
int tts_lift_handle_ctrl_msg(RX_SOCKET socket, int msgId, void *pdata)
{
	...
	case CMD_LIFT_PRINT_POS:
		...
		if (pos < TTS_PRINT_POS_MIN)
		{
			...
			Error(WARN, 0, "%s: set to MIN pos %d.%03d mm", ctrl_cmd_name(msgId), pos / 1000, pos % 1000);
		}
		else if (pos > POS_UP)
		{
			pos = POS_UP;
			Error(WARN, 0, "%s: set to MAX pos %d.%03d mm", ctrl_cmd_name(msgId), pos / 1000, pos % 1000);
		}
