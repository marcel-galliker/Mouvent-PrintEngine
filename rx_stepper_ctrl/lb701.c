...
void lb701_main(int ticks, int menu)
{
	...
	RX_TestTableStatus.info.headUpInput_0 = fpga_input(HEAD_UP_IN);
...
{	
	...
	case CMD_CAP_PRINT_POS:			strcpy(_CmdName, "CMD_CAP_PRINT_POS");
									...
									if (!_CmdRunning && (!RX_TestTableStatus.info.z_in_print || steps!=_PrintPos_Act))
									{
										...
										if (RX_TestTableStatus.info.ref_done) _lb701_move_to_pos(CMD_CAP_PRINT_POS, _PrintPos_New);
	...
	case CMD_CAP_UP_POS:			strcpy(_CmdName, "CMD_CAP_UP_POS");
									if (!_CmdRunning)
									{
										if (RX_TestTableStatus.info.ref_done) _lb701_move_to_pos(CMD_CAP_UP_POS, POS_UP);
										else								  _lb701_do_reference();
									}
									break;
