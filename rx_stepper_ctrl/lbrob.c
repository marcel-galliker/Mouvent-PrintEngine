...
void lbrob_display_status(void)
{
    ...
    if (RX_StepperStatus.robot_used)
    {
        ...
        term_printf("x in reference: \t %d, in=%d\n", RX_StepperStatus.info.x_in_ref, fpga_input(CABLE_PULL_REF));
...
static void _lbrob_do_reference()
{
    ...
    _CmdRunning = CMD_ROB_REFERENCE;
     ...
     TrPrintfL(TRUE, "_lbrob_do_reference: z_in_down=%d, ref_done=%d, SlideInRef=%d", RX_StepperStatus.screwerinfo.z_in_down, RX_StepperStatus.robinfo.ref_done, fpga_input(CABLE_PULL_REF));
...
static void _cln_move_to(int msgId, ERobotFunctions fct, int position)
{
    ...
    if (!_CmdRunning)
    {
        ...
        case rob_fct_move_to_pos:
            ...
            TrPrintfL(TRUE, "_lbrob_move_to_pos 2 cmd=0x%08x, pos=%d", _CmdRunning, pos);
    ...
    if (!RX_StepperStatus.info.moving && !RX_StepperStatus.robinfo.moving && !RX_StepperStatus.screwerinfo.moving)
    {
        ...
        case 1: // new screw
            ...
            if (_ScrewPar.printbar<2)
            {
                ...
                if ( abs(_pos.x - _default.x)>(CORR_MAX+2*CORR_STEP)) _pos.x = _default.x;
                if ( abs(_pos.y - _default.y)>(CORR_MAX+2*CORR_STEP)) _pos.y = _default.y;
       ...
       case 2:  // move slide 
            ...
            _cln_move_to(CMD_ROB_MOVE_POS, rob_fct_move_to_pos, _pos.y+ROB_POSY_FREE);
        ...
        case 3: // move rob to y (move robi and slide simultanious)
            ...
            else if (RX_StepperStatus.robinfo.ref_done && (RX_StepperStatus.info.z_in_screw || RX_StepperStatus.info.z_in_ref || RX_StepperStatus.info.z_in_wash || RX_StepperStatus.info.z_in_cap) && RX_StepperStatus.info.ref_done)
            {
                ...
                Error(LOG, 0, "Robot Move to (x=%d, y=%d)", _pos.x, _pos.y);
        ...
        case 4: // move rob to x
            ...
            TrPrintfL(trace, "_rob_state_machine: State=%d, moveDone=%d, y_in_pos=%d, head=%d/%d-%d", _RobStateMachine_Step, rc_move_done(), RX_StepperStatus.screwerinfo.y_in_pos, _ScrewPar.printbar, _ScrewPar.head, _ScrewPar.axis);
            if (rc_move_xy_error())
            {
                Error(ERR_CONT, 0, "Motor stalled");
                _RobStateMachine_Step=0;
            ...
            {
                TrPrintfL(TRUE, "_rob_state_machine: State=%d, y_in_pos=%d, head=%d/%d-%d", _RobStateMachine_Step, RX_StepperStatus.screwerinfo.y_in_pos, _ScrewPar.printbar, _ScrewPar.head, _ScrewPar.axis);
       ...
       //     if (RX_StepperStatus.screwerinfo.x_in_pos)
        ...
        case 11:
            if (RX_StepperStatus.screwerinfo.z_in_down)
            {
                ...
                {
                    if (_ScrewPar.head<0) ErrorEx(dev_head, _ScrewPar.printbar*RX_StepperCfg.headsPerColor, LOG, 0, "PrintBar %s: screw not found (x=%d, y=%d), trying again",  _AxisName[_ScrewPar.axis], RX_StepperStatus.screw_posX, RX_StepperStatus.posY[0]-RX_StepperStatus.screw_posY);
                    else                  ErrorEx(dev_head, _ScrewPar.printbar*RX_StepperCfg.headsPerColor, LOG, 0, "Head %d %s: screw not found (x=%d, y=%d), trying again", _ScrewPar.head+1, _AxisName[_ScrewPar.axis], RX_StepperStatus.screw_posX , RX_StepperStatus.posY[0]-RX_StepperStatus.screw_posY);
                }
                ...
                Error(LOG, 0, "Robot Move to (x=%d (%d+%d), y=%d (%d+%d)", _pos.x+_corr.x, _pos.x, _corr.x, _pos.y+_corr.y, _pos.y, _corr.y);
                rc_moveto_y(ROB_POSY_FREE, _FL_);
                _cln_move_to(CMD_ROB_MOVE_POS, rob_fct_move_to_pos, _pos.y+_corr.y+ROB_POSY_FREE);
        ...
        case 13:    // screw found 
            ...
            if (_ScrewPar.head<0)
            {
                ...
                if (TRUE || _ScrewFunction==CMD_FIND_ALL_SCREWS) ErrorEx(dev_head, _ScrewPar.printbar*RX_StepperCfg.headsPerColor, LOG, 0, "PrintBar %s: screw found (x=%d, y=%d)",  _AxisName[_ScrewPar.axis],  RX_StepperStatus.screw_posX, pos);
            ...
            else
            {
                ...
                if (TRUE || _ScrewFunction==CMD_FIND_ALL_SCREWS) ErrorEx(dev_head, _ScrewPar.printbar*RX_StepperCfg.headsPerColor, LOG, 0, "Head %d %s: screw found (x=%d, y=%d)", _ScrewPar.head+1, _AxisName[_ScrewPar.axis], RX_StepperStatus.screw_posX, pos);
