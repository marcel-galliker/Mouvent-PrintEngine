...
void lb702_main(int ticks, int menu)
{
	...
	if (_CmdRunning && motors_move_done(MOTOR_Z_BITS)) 
	{
		...
		{
			if (!RX_TestTableStatus.info.headUpInput_0) Error(ERR_CONT, 0, "LB702: Command REFERENCE: End Sensor 1 NOT HIGH");
			if (!RX_TestTableStatus.info.headUpInput_1) Error(ERR_CONT, 0, "LB702: Command REFERENCE: End Sensor 2 NOT HIGH");
...
{	
	...
	case CMD_CAP_PRINT_POS:			strcpy(_CmdName, "CMD_CAP_PRINT_POS");
									...
									if (!_CmdRunning && (!RX_TestTableStatus.info.z_in_print || steps!=_PrintPos_Act))
									{
										...
										if (RX_TestTableStatus.info.ref_done) _lb702_move_to_pos(CMD_CAP_PRINT_POS, _PrintPos_New);
	...
	case CMD_CAP_UP_POS:			strcpy(_CmdName, "CMD_CAP_UP_POS");
									if (!_CmdRunning)
									{
										if (RX_TestTableStatus.info.ref_done) _lb702_move_to_pos(CMD_CAP_UP_POS, -1*_micron_2_steps(RX_StepperCfg.ref_height - 20000));
										else								  _lb702_do_reference();
									}
									break;
