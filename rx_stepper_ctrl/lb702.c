...
void lb702_main(int ticks, int menu)
{
	...
	RX_StepperStatus.posY				= motor_get_step(MOTOR_Z_FRONT);
	
	if(RX_StepperCfg.use_printhead_en)
	{
		int enabled=fpga_input(PRINTHEAD_EN);
		if (RX_StepperStatus.info.printhead_en && !enabled) Error(ERR_ABORT, 0, "LIFT: Allow Head Down signal removed!");
	...
	{
	//	Error(LOG, 0, "done 0x%8x %s", RX_StepperStatus.cmdRunning, _CmdName);
	TrPrintfL(TRUE, "LIFT: done 0x%8x %s [_CmdStep = 0x%8x]", RX_StepperStatus.cmdRunning, _CmdName, _CmdStep);
		RX_StepperStatus.info.moving = FALSE;
        if (RX_StepperStatus.cmdRunning == CMD_LIFT_CALIBRATE)
        {            
            rx_sleep(100);
            switch (_CmdStep)
            {
            case 0:	//--- referenced on top ----------------- 
					if (!RX_StepperStatus.info.headUpInput_0) Error(ERR_CONT, 0, "LIFT: LB702: Command REFERENCE: End Sensor 1 NOT HIGH");
					if (!RX_StepperStatus.info.headUpInput_1) Error(ERR_CONT, 0, "LIFT: LB702: Command REFERENCE: End Sensor 2 NOT HIGH");
                    ...
                    {
                        Error(ERR_CONT, 0, "LIFT: Calibation Error in step%d", _CmdStep);
            ...
            case 1:	//--- motor above tool -----------------------------------------
                    ...
                    {
                        _CmdStep++;
					    Error(ERR_CONT, 0, "LIFT: Calibation continue in step%d", _CmdStep);
                        motors_move_by_step(MOTOR_Z_BITS, &_ParZ_calibrate, -100000, TRUE);
                    }
                    break;
                    
            case 2:	//--- motors blocked at tool -------------------------------------------------
					RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back  = -_incs_2_micron(fpga_encoder_pos(MOTOR_Z_BACK))  + CAL_TOOL_HEIGHT;
					RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front = -_incs_2_micron(fpga_encoder_pos(MOTOR_Z_FRONT)) + CAL_TOOL_HEIGHT;
					Error(LOG, 0, "LIFT: Calibration done: ref_height_back=% 6d, ref_height_front=% 6d", RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back, RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front);
					ctrl_send_2(REP_LIFT_CALIBRATE, sizeof(RX_StepperCfg.robot[RX_StepperCfg.boardNo]), &RX_StepperCfg.robot[RX_StepperCfg.boardNo]);
					RX_StepperStatus.cmdRunning = FALSE;
					break;
            }
        }
        else if (RX_StepperStatus.cmdRunning == CMD_LIFT_REFERENCE) 
		{
			if (!RX_StepperStatus.info.ref_done)
			{
				...
				{
					motors_reset(MOTOR_Z_BITS);
					TrPrintfL(TRUE, "LIFT: CMD_LIFT_REFERENCE done, _Cmd_New=0x%08x", _Cmd_New);
					if (_Cmd_New)
					{
				...
				{
					if (!RX_StepperStatus.info.headUpInput_0) Error(ERR_CONT, 0, "LIFT: LB702: Command REFERENCE: End Sensor BACK NOT HIGH");
					if (!RX_StepperStatus.info.headUpInput_1) Error(ERR_CONT, 0, "LIFT: LB702: Command REFERENCE: End Sensor FRONT NOT HIGH");
					RX_StepperStatus.cmdRunning=FALSE;
					}
				}			
			else
		...
		else if(RX_StepperStatus.cmdRunning == CMD_LIFT_PRINT_POS || RX_StepperStatus.cmdRunning == CMD_LIFT_UP_POS)
		{
			...
			else
			{
				...
				TrPrintf(TRUE, "LIFT: check position offset and correct it!");
				for (motor=MOTOR_Z_BACK; motor<=MOTOR_Z_FRONT; motor++)
				{
					...
					{
						Error(WARN, 0, "LIFT: Motor %s position error: %d", _MotorName[motor], offset[motor] * 10 / STEPS);
						motor_move_by_step(motor, &_ParZ_down, offset[motor]);
						adjustMotors |= 1<<motor;
					}
				}
				if (adjustMotors) motors_start(adjustMotors, TRUE);
		...
		if (!RX_StepperStatus.cmdRunning)
		{
			...
			RX_StepperStatus.info.z_in_cap    = (cmd == CMD_LIFT_CAPPING_POS);
			if (cmd != RX_StepperStatus.cmdRunning)
			{
				TrPrintfL(TRUE, "LIFT: %s: z_in_ref = %d, z_in_up = %d, z_in_print = %d, z_in_cap = %d, robot_used = %d", __func__, RX_StepperStatus.info.z_in_ref,
								RX_StepperStatus.info.z_in_up, RX_StepperStatus.info.z_in_print, RX_StepperStatus.info.z_in_cap, RX_StepperStatus.robot_used);
			}
		}
	}
	...
	if (RX_StepperStatus.robot_used && _CmdRunningRobi)
	{
		...
		if (RX_StepperStatus.robinfo.ref_done && loc_new_cmd)
		{
			...
			default: Error(ERR_CONT, 0, "LIFT: LB702_MAIN: Command 0x%08x not implemented", loc_new_cmd); break;
			}
		}
		loc_new_cmd = FALSE;	
	}
...
static void _lb702_display_status(void)
{
	...
	term_printf("Ref Height back(steps):      %06d  Print Height:     %06d\n", _micron_2_steps(RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back),  _micron_2_steps(_PrintHeight));
	term_printf("Ref Height front(steps):     %06d  Print Height:     %06d\n", _micron_2_steps(RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front), _micron_2_steps(_PrintHeight));
	...
	term_printf("Motor BACK pos(um):      %06d  Motor FRONT pos(um):     %06d\n", abs(Fpga.encoder[0].pos) / 8, abs(Fpga.encoder[1].pos) / 8);
...
static void _lb702_handle_menu(char *str)
{
	...
	if (str)
	{
		...
		case 'C': lb702_handle_ctrl_msg(INVALID_SOCKET, CMD_LIFT_CALIBRATE,	NULL); break;            
...
static void _lb702_move_to_pos(int cmd, int pos0, int pos1)
{
	TrPrintfL(TRUE, "LIFT: cmd = %d, pos0 = %d, pos1 = %d, print_height = %d, ref_height_back = %d, ref_height_front = %d", cmd, pos0, pos1,
					RX_StepperCfg.print_height, RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back, RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front);
	int adjust=0;
	...
	else if((cmd == CMD_LIFT_PRINT_POS)||(cmd == CMD_LIFT_UP_POS)||(cmd == CMD_LIFT_REFERENCE)||(cmd == CMD_LIFT_CAPPING_POS))
	{
		RX_StepperStatus.info.moving = TRUE;
...
{		
	...
	switch(msgId)
	{
	case CMD_LIFT_STOP:				TrPrintfL(TRUE, "LIFT: CMD_LIFT_STOP");
									strcpy(_CmdName, "CMD_LIFT_STOP");
									motors_stop(MOTOR_Z_BITS);
	...
	case CMD_LIFT_REFERENCE:		TrPrintfL(TRUE, "LIFT: CMD_LIFT_REFERENCE");
    ...
    case CMD_LIFT_CALIBRATE:			/*
									...
									motors_move_by_step	(MOTOR_Z_BITS,  &_ParRef, 500000, TRUE);
									*/
									TrPrintfL(TRUE, "LIFT: CMD_LIFT_CALIBRATE");
									Error(WARN, 0, "LIFT: Calibration not available yet");
									break;

	case CMD_LIFT_PRINT_POS:		TrPrintfL(TRUE, "LIFT: CMD_LIFT_PRINT_POS");
									strcpy(_CmdName, "CMD_LIFT_PRINT_POS");
								//	Error(LOG, 0, "starting 0x%08x %s", msgId, _CmdName);
									if(!RX_StepperStatus.info.printhead_en) return ErrorFlag(ERR_ABORT, &_ErrorFlags, 0x00001, 0, "LIFT: Allow Head Down signal not set!");
									_PrintHeight   = (*((INT32*)pdata));
									_Step=0;
									if (RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back < 10000) Error(ERR_ABORT, 0, "LIFT: Reference Height back must be > 10mm");
									else if (RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front < 10000) Error(ERR_ABORT, 0, "LIFT Reference Height must be > 10mm");
									...
									else if(!RX_StepperStatus.cmdRunning || RX_StepperStatus.cmdRunning==CMD_LIFT_REFERENCE)
									{
										...
										{
											if (RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back < 90000) Error(WARN, 0, "LIFT: Reference Height back small");
											if (RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front < 90000) Error(WARN, 0, "LIFT: Reference Height front small");
											...
											{
												_Cmd_New      = msgId;
												TrPrintf(TRUE, "LIFT: Start REF, _Cmd_New=0x%08x", _Cmd_New);
												_PrintPos_New[MOTOR_Z_BACK]  = val0;
												_PrintPos_New[MOTOR_Z_FRONT] = val1;
	...
	case CMD_LIFT_UP_POS:			TrPrintfL(TRUE, "LIFT: CMD_LIFT_UP_POS");
	...
	case CMD_LIFT_CAPPING_POS:		TrPrintfL(TRUE, "LIFT: CMD_LIFT_CAPPING_POS");
									strcpy(_CmdName, "CMD_LIFT_CAPPING_POS");
								//	Error(LOG, 0, "starting 0x%08x %s", msgId, _CmdName);
									if		(RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back  < 10000) Error(ERR_ABORT, 0, "LIFT: Reference Height back must be > 10mm");
									else if (RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front < 10000) Error(ERR_ABORT, 0, "LIFT: Reference Height front must be > 10mm");
									else if (abs(RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_front - RX_StepperCfg.robot[RX_StepperCfg.boardNo].ref_height_back) > MAX_ALIGN) 
										Error(ERR_ABORT, 0, "LIFT: Reference Height front/back too differents (> 10mm)");		
									else if (RX_StepperCfg.robot[RX_StepperCfg.boardNo].cap_height > 15000) Error(ERR_ABORT, 0, "LIFT: Capping Height must be < 15mm");
									else if (!RX_StepperStatus.cmdRunning)
									{
										RX_StepperStatus.cmdRunning  = msgId;
										if (RX_StepperCfg.robot[RX_StepperCfg.boardNo].cap_height < 5000) Error(WARN, 0, "LIFT: Reference Height back should be > 5mm");
	...
	case CMD_ERROR_RESET:			TrPrintfL(TRUE, "LIFT: CMD_ERROR_RESET");
