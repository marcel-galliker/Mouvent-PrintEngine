...
void tx801_main(int ticks, int menu)
{
	...
	if (_CmdRunning && motors_move_done(MOTOR_Z_BITS)) 
	{
        ...
        RX_StepperStatus.robinfo.z_in_vacuum	= (_CmdRunning == CMD_CAP_VACUUM_POS) || (_CmdRunning == CMD_CAP_VACUUM_HIGH_POS);
...
int tx801_menu(void)
{
	...
	if (term_get_str(str, sizeof(str)))
	{
		...
		case 'e': _tx801_motor_down(INVALID_SOCKET, CMD_CAP_CAPPING_POS, _WashHight); break;
...
{	
	...
	case CMD_CAP_WIPE_POS:			if (!_CmdRunning)
									{
										_CmdRunning = msgId;
										RX_StepperStatus.info.moving = TRUE;
	...
	case CMD_CAP_VACUUM_POS:		if (!_CmdRunning)
									{
										_CmdRunning = msgId;
										RX_StepperStatus.info.moving = TRUE;
	...
	case CMD_CAP_VACUUM_HIGH_POS:	if (!_CmdRunning)
									{
										_CmdRunning = msgId;
										RX_StepperStatus.info.moving = TRUE;
