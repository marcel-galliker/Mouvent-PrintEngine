...
void txrob_main(int ticks, int menu)
{
	...
	if (RX_StepperStatus.cmdRunning && motors_move_done(MOTOR_ALL_BITS) && (_TimeCnt == 0))
	{
		...
		if ((RX_StepperStatus.cmdRunning != CMD_ROB_REFERENCE) && (RX_StepperStatus.cmdRunning != CMD_ROB_ROT_REF) && (RX_StepperStatus.cmdRunning != CMD_ROB_ROT_REF2) && (RX_StepperStatus.cmdRunning != FALSE))
		{		
		...
		if (RX_StepperStatus.cmdRunning == CMD_ROB_ROT_REF)
		{
		...
		if (RX_StepperStatus.cmdRunning == CMD_ROB_STOP)
		{
			_ResetRetried = 0;
		}
		...
		if (RX_StepperStatus.cmdRunning == CMD_ROB_SHIFT_MOV) 
		{
			...
			case 3: // End
				...
				case rob_fct_wash:	// Wet wipe
					//loc_new_cmd = CMD_ROB_EMPTY_WASTE; 
					break;
		...
		if (RX_StepperStatus.cmdRunning == CMD_ROB_VACUUM) 
		{
			loc_new_cmd = CMD_ROB_EMPTY_WASTE;
		...
		if (RX_StepperStatus.cmdRunning == CMD_ROB_EMPTY_WASTE) 
		{
			Fpga.par->output &= ~PUMP_WASTE_BASE; // Waste base OFF
			Fpga.par->output &= ~PUMP_WASTE_VAC; // Waste base OFF
			Fpga.par->output &= ~VAC_ON; // VACUUM OFF
		...
		case CMD_ROB_EMPTY_WASTE: txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_EMPTY_WASTE, NULL); break;
		case CMD_ROB_VACUUM:	txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_VACUUM, NULL); break;
...
int txrob_menu(void)
{
	...
	if (term_get_str(str, sizeof(str)))
	{
		...
		case 'c': pos = 0; txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_MOVE_POS, &pos); break;
		case 'e': pos = 1; txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_MOVE_POS, &pos); break;
		case 'a': pos = 2; txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_MOVE_POS, &pos); break;
		case 'w': pos = 3; txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_MOVE_POS, &pos); break;
		case 'b': pos = 3; txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_SHIFT_MOV, &pos); break;
		case 'l': pos = 1; txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_SHIFT_LEFT, &pos); break;
...
{	
	...
	case CMD_ROB_REFERENCE:			strcpy(_CmdName, "CMD_ROB_REFERENCE");
		...
		if (RX_StepperStatus.cmdRunning && RX_StepperStatus.cmdRunning != CMD_FLUID_CTRL_MODE){ txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_STOP, NULL); _NewCmd = CMD_ROB_REFERENCE; break; }
	...
	case CMD_ROB_SHIFT_REF:			strcpy(_CmdName, "CMD_ROB_SHIFT_REF");
		if (RX_StepperStatus.cmdRunning && RX_StepperStatus.cmdRunning != CMD_FLUID_CTRL_MODE){ txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_STOP, NULL); _NewCmd = CMD_ROB_SHIFT_REF; break; }
		motor_reset(MOTOR_SHIFT); // to recover from move count missalignment
		Fpga.par->output &= ~RO_ALL_OUTPUTS;
		RX_StepperStatus.robinfo.ref_done = FALSE;
		RX_StepperStatus.robinfo.moving = TRUE;
	...
	case CMD_ROB_ROT_REF:			strcpy(_CmdName, "CMD_ROB_ROT_REF");
		if (RX_StepperStatus.cmdRunning && RX_StepperStatus.cmdRunning != CMD_FLUID_CTRL_MODE){ txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_STOP, NULL); _NewCmd = CMD_ROB_ROT_REF; break; }
		motor_reset(MOTOR_ROT); // to recover from move count missalignment
		Fpga.par->output &= ~RO_ALL_OUTPUTS;
		RX_StepperStatus.robinfo.ref_done = FALSE;
		RX_StepperStatus.robinfo.moving = TRUE;
	...
	case CMD_ROB_ROT_REF2:	strcpy(_CmdName, "CMD_ROB_ROT_REF2");
		if (RX_StepperStatus.cmdRunning && RX_StepperStatus.cmdRunning != CMD_FLUID_CTRL_MODE){ txrob_handle_ctrl_msg(INVALID_SOCKET, CMD_ROB_STOP, NULL); _NewCmd = CMD_ROB_ROT_REF2; break; }
		motor_reset(MOTOR_ROT); // to recover from move count missalignment
		Fpga.par->output &= ~RO_ALL_OUTPUTS;
		RX_StepperStatus.robinfo.ref_done = FALSE;
		RX_StepperStatus.robinfo.moving = TRUE;
	...
	case CMD_ROB_EMPTY_WASTE :		strcpy(_CmdName, "CMD_ROB_EMPTY_WASTE");
		if (!RX_StepperStatus.cmdRunning || RX_StepperStatus.cmdRunning == CMD_FLUID_CTRL_MODE)
		{
	...
	case CMD_ROB_VACUUM :		strcpy(_CmdName, "CMD_ROB_VACUUM");
		if (!RX_StepperStatus.cmdRunning || RX_StepperStatus.cmdRunning == CMD_FLUID_CTRL_MODE)
		{
			Fpga.par->output |= PUMP_WASTE_VAC; // Waste base ON
			Fpga.par->output |= VAC_ON; // VACUUM ON
