...
void sa_main(int ticks, int menu)
{
	...
	if (RX_StepperStatus.cmdRunning && motors_move_done(MOTOR_Z_BITS)) 
	{
		...
		{
			if (_PrintPos_New==POS_UP) _sa_move_to_pos(CMD_LIFT_UP_POS,    _PrintPos_New);
			else                       _sa_move_to_pos(CMD_LIFT_PRINT_POS, _PrintPos_New);
...
int sa_menu(void)
{
	...
	if (term_get_str(str, sizeof(str)))
	{
		switch (str[0])
		{
		case 's': sa_handle_ctrl_msg(INVALID_SOCKET, CMD_LIFT_STOP,			NULL); break;
		case 'R': sa_handle_ctrl_msg(INVALID_SOCKET, CMD_LIFT_REFERENCE,		NULL); break;
		case 'r': motor_reset(atoi(&str[1]));										   break;
		case 'c': sa_handle_ctrl_msg(INVALID_SOCKET, CMD_LIFT_CAPPING_POS,	NULL); break;
		case 'p': sa_handle_ctrl_msg(INVALID_SOCKET, CMD_LIFT_PRINT_POS,		&pos); break;
		case 'u': sa_handle_ctrl_msg(INVALID_SOCKET, CMD_LIFT_UP_POS,		NULL); break;
...
//	commands
//	CMD_LIFT_REFERENCE:	top stop	(no parameter)
//	CMD_LIFT_UP_POS:	top stop	(no parameter)
//	CMD_LIFT_PRINT_POS:	bottom stop (no parameter)
//	
//	CMD_ROB_REFERENCE:	refenrence Scanner (stop at machine side, position=0)
//	CMD_ROB_MOVE_POS:	position scanner (position in microns)
	...
	case CMD_LIFT_PRINT_POS:			strcpy(_CmdName, "CMD_LIFT_PRINT_POS");
									...
									if (!RX_StepperStatus.cmdRunning && (!RX_StepperStatus.info.z_in_print || steps!=_PrintPos_Act))
									{
										...
										if (RX_StepperStatus.info.ref_done) _sa_move_to_pos(CMD_LIFT_PRINT_POS, _PrintPos_New);
	...
	case CMD_LIFT_UP_POS:			strcpy(_CmdName, "CMD_LIFT_UP_POS");
									...
									{
										if (RX_StepperStatus.info.ref_done) _sa_move_to_pos(CMD_LIFT_UP_POS, POS_UP);
										else								_sa_do_reference();
									}
									break;
