	...
	for(isNo = 0 ; isNo < NIOS_INK_SUPPLY_CNT ; isNo++)
	{
		...
		/*		if(pRX_Status->ink_supply[isNo].ctrl_state == ctrl_print)
			...
			case ctrl_print_run:
				...
				_pump_ctrl(isNo, _PressureSetpoint[isNo], TRUE);
				break;
...
static void _pump_ctrl(INT32 isNo, INT32 pressure_target, INT32 print_mode)
{
	...
	//-----------------------------------------------------------------------------------------
		...
		if(print_mode == 1) _InkSupply[isNo].pid_Pump.Setpoint = pRX_Status->ink_supply[isNo].IS_Pressure_Setpoint;
		...
		if(print_mode == 2) _InkSupply[isNo].pid_Pump.P = 200;
