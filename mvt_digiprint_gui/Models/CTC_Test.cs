...
namespace RX_DigiPrint.Models
{
	public class CTC_Test: RxBindable
	{
		...
		public void Start()
		{
			for (int head = 0; head < HEADS; head++) 
			{
				if (Overall.State[head]!=EN_State.failed && head<RxGlobals.HeadStat.List.Count && RxGlobals.HeadStat.List[head].Connected) 
					SetHeadState(head, EN_State.running);
				else SetHeadState(head, EN_State.undef);
			}
		}
