...
/* Provide a single point for memory planes access.
*/

int planealloc(const gs_memory_t *mem, int planes, int width, int heigth, char ***buf)
{
	gs_lib_ctx_t *pio = mem->gs_lib_ctx;

	if (pio->planealloc_fn) {
		if ((planes == 0) || (width*heigth == 0)) {
			errwrite(mem, "planealloc param error", 23);
			return 0;
		}
		return (*pio->planealloc_fn)(pio->caller_handle, planes, width, heigth, buf);
	}
	return 0;
}


int planedone(const gs_memory_t *mem)
{
	gs_lib_ctx_t *pio = mem->gs_lib_ctx;


	if (pio->planedone_fn) {
		return (*pio->planedone_fn)(pio->caller_handle);
	}
	return 0;
}


bool planefndefine(const gs_memory_t *mem)
{
	gs_lib_ctx_t *pio = mem->gs_lib_ctx;


	if ((pio->planealloc_fn) && (pio->planedone_fn))
		return true;
