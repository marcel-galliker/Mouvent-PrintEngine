...
/* Set the callback functions for memio */
GSDLLEXPORT int GSDLLAPI
gsapi_set_planeio(void *instance,
	int(GSDLLCALL *planealloc_fn)(void *caller_handle, int planes, int width, int heigth, char ***buf),
	int(GSDLLCALL *planedone_fn)(void *caller_handle))
{
	gs_lib_ctx_t *ctx = (gs_lib_ctx_t *)instance;
	if (instance == NULL)
		return gs_error_Fatal;
	ctx->planealloc_fn = planealloc_fn;
	ctx->planedone_fn = planedone_fn;
	return 0;
}
