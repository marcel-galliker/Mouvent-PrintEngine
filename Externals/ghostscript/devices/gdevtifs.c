...
tiff_print_page(gx_device_printer *dev, TIFF *tif, int min_feature_size)
{
	...
	if (!planefndefine(dev->memory)) 
		code = TIFFCheckpointDirectory(tif);
	else
		planealloc(dev->memory, 4, (int)dev->width, dev->height, &buffer);
        ...
        if (row - line_lag >= 0) {
			...
			if (!planefndefine(dev->memory)) {
				...
				for (int x = 0; x < dev->width; x++) {
					*(buffer[0] + (row - line_lag)*dev->width + x) = data[(x*4)];
					*(buffer[1] + (row - line_lag)*dev->width + x) = data[(x * 4) + 1];
					*(buffer[2] + (row - line_lag)*dev->width + x) = data[(x * 4) + 2];
					*(buffer[3] + (row - line_lag)*dev->width + x) = data[(x * 4) + 3];
				}
			}
        }
    }
