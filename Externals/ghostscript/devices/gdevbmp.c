...
bmp_cmyk_print_page(gx_device_printer * pdev, FILE * file)
{
	...
	if (planefndefine(pdev->memory)) {
		planealloc(pdev->memory, 4, (int)pdev->width, pdev->height, &buffer);
		for (plane = 0; plane <= 3; ++plane) {
			...
			for (y = 0; y <pdev->height; y++) {
				...
				if (code < 0)
					goto done;
				memcpy(buffer[plane] + y*pdev->width, actual_data, pdev->width);
			}
		}
		planedone(pdev->memory);
	}
	else {
