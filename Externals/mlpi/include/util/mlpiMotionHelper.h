...
// compiling under Mac OS, iOS, ... environment
...
public:
  ...
  MLPI_MOTION_BIT_ACCESS(SupportsParametrization, 15);  //!< Axis supports Parametrization mode. (see @ref mlpiSetCondition)
...
//! The following struct helps to decode the axis condition as given by
//! mlpiMotionGetAxisCondition
//!
//! @par Example:
//! See @ref mlpiMotionGetAxisCondition
...
inline MLPIRESULT utilMotionWait(MLPIHANDLE connection, MlpiAxisRef axis, MLPIMOTIONHANDLE motionHandle, ULONG timeout=MLPI_INFINITE)
{
  ...
  while( !(status.done || status.error) && (timeout==MLPI_INFINITE || timeGone<timeout) )
  {
    ...
    Sleep(REFRESH_TIME);
    #elif defined (TARGET_OS_APPLE)
    // compiling under MACOS environment
    #elif defined (TARGET_OS_ANDROID)
    // compiling under Android environment
    #elif defined (TARGET_OS_LINUX)
    // compiling under Linux environment
